...
# DB:table(db_dong_price) -> R:dataframe
query.str.dong <- paste("select * from db_dong_price ", sep="")
...
### 선 그래프
dong <- ggplot(dong_apt_price_all, aes(x=거래시점, y=평균거래가, color = 동이름)) +
  geom_line(stat="identity", size= 1.25)+
  labs(title = '도봉구 동별 평균 거래금액') +
...
db_dong_apt_price <- ggplotly(dong)

### 그래프 저장
htmlwidgets::saveWidget(db_dong_apt_price,"./team_df/db_dong_apt_price.html", selfcontained = F)

### 실행 - cmd 
# 1) 파일 위치 => cd C:\Apache24\htdocs\team
# 2) Rscript 위치 + R파일 => "C:/Program Files/R/R-4.2.2/bin/Rscript.exe" db_dong_trade.R
