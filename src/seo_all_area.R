...
### 선 그래프
avg_trade_by_area <- ggplot(seo_apt_price, aes(x = 날짜, y = 평당평균거래가)) +
  geom_line(color = 'blue', size= 1.25)+
    labs(title = '서초구 전체 평당 평균거래가') +
...
seo_price_area <-ggplotly(avg_trade_by_area)

### 그래프 저장
htmlwidgets::saveWidget(seo_price_area,"./team_df/seo_price_area.html", selfcontained = F)

### 실행 - cmd 
# 1) 파일 위치 => cd C:\Apache24\htdocs\team
# 2) Rscript 위치 + R파일 => "C:/Program Files/R/R-4.2.2/bin/Rscript.exe" seo_all_area.R