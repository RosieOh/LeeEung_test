...
### 선 그래프
gn_area <- ggplot(gn_apt_price, aes(x=거래시점, y=평당평균거래가)) +
  geom_line(color = 'blue', size= 1.25)+
  labs(title = '강남구 전체 평당 평균거래가') +
...
gn_price_area <- ggplotly(gn_area)

### 그래프 저장
htmlwidgets::saveWidget(gn_price_area,"./team_df/gn_price_area.html", selfcontained = F)

### 실행 - cmd 
# 1) 파일 위치 => cd C:\Apache24\htdocs\team
# 2) Rscript 위치 + R파일 => "C:/Program Files/R/R-4.2.2/bin/Rscript.exe" gn_all_area.R
