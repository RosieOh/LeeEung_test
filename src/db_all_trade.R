...
# DB:table(db_price_flow) -> R:dataframe
...
### 선 그래프
db_trade <- ggplot(db_apt_price, aes(x=거래시점, y=평균거래가)) +
  geom_line(color = 'red', size= 1.25)+
  labs(title = '도봉구 전체 평균 거래금액') +
...
db_price <- ggplotly(db_trade)

### 그래프 저장
htmlwidgets::saveWidget(db_price,"./team_df/db_price.html", selfcontained = F)

### 실행 - cmd 
# 1) 파일 위치 => cd C:\Apache24\htdocs\team
# 2) Rscript 위치 + R파일 => "C:/Program Files/R/R-4.2.2/bin/Rscript.exe" db_all_trade.R
